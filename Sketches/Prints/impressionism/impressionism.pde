...
void settings(){
  ...
  img = loadImage("pixelart/earth_pixel.png");
...
void setup() {
  ...
  beginRecord(PDF, "earth_fire.pdf");
...
void vanGogh() {
  ...
  for (int x = 0; x < img.width; x+=1) {
    for (int y = 0; y < img.height; y+=1) {
      ...
      line(x + random(0,1) * random(-20,40),y ,x + fractionLen,y + (fractionLen * random(0,1)));
      singlePix +=1;
